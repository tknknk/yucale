...
export default function SurveyForm({ onSubmit, onCancel, isLoading = false, initialData }: SurveyFormProps) {
  ...
  const {
    ...
    defaultValues: {
      ...
      enableCheckbox: initialData?.enableCheckbox ?? false,
      checkboxLabel: initialData?.checkboxLabel ?? '',
    ...
    const initializeForm = async () => {
      try {
        if (isEditMode && initialData) {
          ...
          if (data.defaultCheckboxLabel) {
            setValue('checkboxLabel', data.defaultCheckboxLabel);
          }
        }
      } catch (err) {
  ...
  const onFormSubmit = async (data: SurveyFormData) => {
    ...
    const request: CreateSurveyRequest = {
      ...
      enableCheckbox: data.enableCheckbox,
      checkboxLabel: data.enableCheckbox ? (data.checkboxLabel?.trim() || undefined) : undefined,
      ...
      <div>
        <div className="flex items-center">
          <input
            type="checkbox"
            id="enableCheckbox"
            {...register('enableCheckbox')}
            className="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500"
            disabled={isLoading || isSubmitting}
          />
          <label htmlFor="enableCheckbox" className="ml-2 block text-sm text-gray-900">
            チェックボックスを追加する
          </label>
        </div>
        {watch('enableCheckbox') && (
          <div className="mt-2">
            <label htmlFor="checkboxLabel" className="block text-sm font-medium text-gray-800">
              チェックボックスのラベル <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              id="checkboxLabel"
              {...register('checkboxLabel', {
                validate: (value, formValues) =>
                  !formValues.enableCheckbox ||
                  (!!value && value.trim().length > 0) ||
                  'チェックボックスのラベルを入力してください',
                maxLength: { value: 100, message: 'ラベルは100文字以内で入力してください' },
              })}
              className={`mt-1 block w-full rounded-md shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm px-3 py-2 ${
                errors.checkboxLabel ? 'border-red-500 border' : 'border border-gray-300'
              }`}
              placeholder="例: 懇親会に参加する"
              disabled={isLoading || isSubmitting}
            />
            {errors.checkboxLabel && <p className="mt-1 text-sm text-red-600">{errors.checkboxLabel.message}</p>}
            <p className="mt-1 text-xs text-gray-800">回答者に各スケジュールで表示されるチェックボックスの文言です</p>
          </div>
        )}
      </div>
