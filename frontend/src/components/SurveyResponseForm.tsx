...
// スケジュールの場所・曲・詳細を表示する。ScheduleCardと同じアイコンを使う。
function ScheduleDetailInfo({ detail }: { detail: SurveyDetail }) {
  if (!detail.scheduleLocation && !detail.scheduleSong && !detail.scheduleDescription) {
    return null;
  }
  return (
    <div className="space-y-1 text-sm text-gray-800">
      {/* Location */}
      {detail.scheduleLocation && (
